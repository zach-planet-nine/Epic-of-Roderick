...
- (id)initWithImageNamed:(NSString*)aName filter:(GLenum)aFilter {
    ...
    if ([[GameController sharedGameController].teorPSS imageForKey:aName] != nil) {
        return [[[[GameController sharedGameController].teorPSS imageForKey:aName] imageDuplicate] retain];
    }
    self = [super init];
	if (self != nil) {
        // Initialize the common properties for this image
        [self initializeImage:aName filter:aFilter];
		renderPoint = CGPointZero;
...
- (void)initializeImage:(NSString*)aImageName filter:(GLenum)aFilter {
	...
	self.texture = [sharedTextureManager textureWithFileName:self.imageFileName filter:aFilter];
